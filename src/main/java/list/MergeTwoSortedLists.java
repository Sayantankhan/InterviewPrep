...
public class MergeTwoSortedLists {
    ...
    public static void mergerTwoList(List<Integer> l1, List<Integer> l2) {
        ...
        // if size(l1) = m, size(l2) = n; Time Complexity = O(mn log mn); Space = O(mn)
        watch.start();
        ...
        // Option2 : As the lists are sorted; we can use a min heap and store the elements from
        // each iteration and heap will give us the min
        // if size(l1) = m, size(l2) = n; no of sorted list -> 2;
        // Time Complexity = O(mn log2) -> O(mn)
        watch.start();
    ...
    public static void main(String[] args) {
        List<Integer> even = IntStream.range(1, 1000).filter(x -> x%2 == 0).boxed().collect(Collectors.toList());
        List<Integer> odd = IntStream.range(1, 1000).filter(x -> x%2 != 0).boxed().collect(Collectors.toList());
